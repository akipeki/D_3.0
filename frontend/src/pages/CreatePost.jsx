...
if (form.prompt) {
  try {
    ...
    const response = await fetch ('https://dalle-f98l.onrender.com/api/v1/dalle', {
    method: 'POST',
    headers: {
...
if(form.prompt && form.photo) {
  setLoading(true);
try {
  const response = await fetch('https://dalle-f98l.onrender.com/api/v1/post', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
  ...
  return (
    <section className='max-w-7xl mx-auto'>
     ...
     <p className="mt-2 text-[#665e75] text-[16px] max-w[500px]">
      Create imaginative and visually stunning images through the DALL-E AI and share them with community</p>
     </div> 
...
focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center'>
 ...
 {generatingImg && (
  ...
  justify-center items-center bg-[rgba(0,0,0,0.5)] rounded-lg'> 
    ...
    <div className='mt-10'>
      <p className='mt-2 text-[#665e75] text-[14px]'>
